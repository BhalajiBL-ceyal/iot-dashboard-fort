...
import { Activity, Server, Wifi, WifiOff, LayoutGrid, Trash2, GripVertical, Gauge, BarChart3, Timer, Battery, MapPin, TrendingUp, ZoomIn, ZoomOut } from 'lucide-react';
...
const NumericCard = ({ value, label, deviceId, unit, threshold }) => {
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200">
      <div className="p-3 border-b border-gray-100">
        <div className="text-xs font-semibold text-gray-700 truncate">{label}</div>
        <div className="text-xs text-gray-500 truncate">{deviceId}</div>
      </div>
      <div className="p-4 flex flex-col items-center justify-center h-[calc(100%-60px)]">
        <div className={`text-3xl lg:text-4xl font-bold ${isWarning ? 'text-red-600' : 'text-blue-600'}`}>
          {displayValue}
        </div>
        {unit && <div className="text-xs text-gray-600 mt-2">{unit}</div>}
      </div>
    </div>
  );
};
    ...
    <div className="h-full w-full bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200">
      ...
      <div className="p-4 flex flex-col justify-center h-[calc(100%-60px)]">
        <div className="text-center mb-3">
          ...
          {unit && <span className="text-xs text-gray-600 ml-2">{unit}</span>}
        </div>
...
const LineChartWidget = ({ history, label, deviceId }) => {
  ...
  if (chartData.length === 0) {
    return (
      <div className="h-full w-full bg-white rounded-lg shadow-md border border-gray-200 flex items-center justify-center">
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md border border-gray-200">
      <div className="p-2 border-b text-xs font-semibold truncate">{label}</div>
...
const StatusIndicator = ({ status, deviceId }) => {
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow border border-gray-200">
      <div className="p-3 border-b border-gray-100">
        <div className="text-xs font-semibold text-gray-700">Device Status</div>
        <div className="text-xs text-gray-500 truncate">{deviceId}</div>
      </div>
      <div className="p-4 flex flex-col items-center justify-center h-[calc(100%-60px)]">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isOnline ? 'bg-green-100' : 'bg-gray-100'}`}>
          <Activity size={24} className={isOnline ? 'text-green-600 animate-pulse' : 'text-gray-400'} />
        </div>
        <div className={`text-sm font-semibold mt-3 ${isOnline ? 'text-green-600' : 'text-gray-500'}`}>
...
const AlarmWidget = ({ value, label, deviceId, threshold }) => {
  ...
  return (
    <div className={`h-full w-full rounded-lg shadow-md p-4 ${isAlarm ? 'bg-red-600 text-white' : 'bg-green-500 text-white'
      }`}>
...
const BatteryWidget = ({ value, deviceId }) => {
  ...
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md p-4">
      ...
      <div className="text-3xl font-bold mt-2">{percent.toFixed(0)}%</div>
      <div className="bg-gray-200 h-2 rounded mt-3">
        <div
          className={`h-2 rounded ${percent > 50 ? 'bg-green-500' : 'bg-red-500'
            }`}
          style={{ width: `${percent}%` }}
        />
      </div>
...
const StatusWidget = ({ online, deviceId }) => (
  <div className={`h-full w-full rounded-lg shadow-md p-4 ${online ? 'bg-green-500' : 'bg-gray-500'
    } text-white`}>
    <div className="text-xs truncate">{deviceId}</div>
...
const MultiMetricWidget = ({ metrics = [], deviceId, telemetryData }) => {
  return (
    <div className="h-full w-full bg-white rounded-lg shadow-md p-4 overflow-auto">
      <div className="text-xs text-gray-500 mb-2 truncate">{deviceId}</div>

      {metrics.length === 0 ? (
        <div className="text-gray-400 text-xs">No metrics configured</div>
      ) : (
        metrics.map((m, i) => (
...
const WidgetLibraryItem = ({ type, icon: Icon, label, onDragStart }) => {
  return (
    <div
      ...
      className="flex items-center gap-3 p-3 bg-slate-700 rounded cursor-grab active:cursor-grabbing hover:bg-slate-600 transition-colors"
    >
      <Icon size={18} />
...
export default function IoTDashboard() {
  ...
  return (
    <div className="h-screen flex bg-gray-50">
      {/* Sidebar */}
      <div className="w-72 bg-slate-800 text-white flex flex-col shadow-xl overflow-hidden">
            ...
            <div className="space-y-2">
              {devices.map(device => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg transition-all ${selectedDevice === device.device_id ? 'bg-blue-600 shadow-lg' : 'bg-slate-700 hover:bg-slate-600'}`}
                >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            // Calculate position in grid using dynamic width
            ...
            console.log('Manual drop handler triggered', { x, y, draggedWidgetType, containerWidth, cellWidth });

            setPendingWidget({
              type: draggedWidgetType,
              x: Math.max(0, Math.min(x, 8)), // Keep within bounds
              y: Math.max(0, y),
              w: 4,
              h: 3,
            });
          ...
          >
            <GridLayout
              ...
              onDrop={(layout, item, e) => {
                console.log('Drop event triggered!', { layout, item, draggedWidgetType, selectedDevice });
                onGridDrop(layout, item, e);
              }}
            ...
            >
              {widgets.length === 0 ? (
                <div
                  key="empty"
                  data-grid={{ x: 0, y: 0, w: 12, h: 8, static: true }}
              ...
              {widgets.map(widget => (
                <div key={widget.id} className="relative group bg-transparent">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 bg-red-600 text-white p-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
